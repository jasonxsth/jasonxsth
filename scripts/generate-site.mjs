...
const caseCard = (project, prefix, index) => {
  ...
  return `<a class="case-card case-card-${index + 1}" href="${href(prefix, `/portfolio/${project.slug}/`)}" data-track="portfolio_open" data-case-id="${project.id}" data-audience="${project.audience}" data-tags="${escapeHtml(data.tags.join(','))}">
...
const renderHome = () => {
  ...
  const prefix = '';
  const brandCases = cases.filter((project) => project.audience === 'brand');
  const designerCases = cases.filter((project) => project.audience === 'designer');
  const selected = [brandCases[0], designerCases[0], brandCases[1], brandCases[2], designerCases[1], brandCases[3]];
  ...
  const body = `<main id="content">
    ...
    <section class="positioning" aria-labelledby="position-title">
      <div class="section-head"><p class="section-kicker">Проектная студия · интерьерная индустрия</p><span>01</span></div>
    ...
    <section class="audience-section" aria-labelledby="audience-title">
      ...
      <div class="audience-split">
        <a class="audience-panel audience-brand" href="${href(prefix, '/b2b/')}" data-track="audience_select" data-audience="brand">
          ${image(prefix, 'assets/projects/dark-bathroom.webp', 'Визуализация продукта в интерьере')}
        ...
        <a class="audience-panel audience-designers" href="${href(prefix, '/designers/')}" data-track="audience_select" data-audience="designer">
          ${image(prefix, 'assets/projects/calm-bedroom.webp', 'Визуализация жилого интерьера')}
    ...
    <section class="task-switcher brand-task-switcher" aria-labelledby="brand-tasks-title" data-task-switcher>
      <div class="task-visuals">${['assets/projects/dark-bathroom.webp', 'assets/projects/bathroom-study.webp', 'assets/projects/collage-bathroom.webp', 'assets/projects/material-bathroom-green.webp'].map((path, index) => `<figure data-task-preview="${index}"${index === 0 ? ' class="is-active"' : ''}>${image(prefix, path, brandTasks[index][0])}</figure>`).join('')}</div>
    ...
    <section class="designer-bridge" aria-labelledby="designer-bridge-title" data-header-theme="dark">
      ...
      <figure>${image(prefix, 'assets/projects/calm-bedroom.webp', 'Жилой интерьер — визуализация RENDART')}</figure>
    ...
    <section class="rnd-story" aria-labelledby="rnd-title" data-rnd-story data-header-theme="dark">
      <div class="rnd-stage">
        ...
        <div class="rnd-media">
          ...
          <figure data-rnd-image="1">${image(prefix, 'assets/projects/material-precision-bedroom.webp', 'Финальная интерьерная визуализация')}</figure>
...
const innerHero = ({ prefix, label, title, body, imagePath, imageAlt, primary, secondary, className = '' }) => `<section class="inner-hero ${className}">
  <div class="inner-hero-copy"><p class="section-kicker">${escapeHtml(label)}</p><h1>${escapeHtml(title)}</h1><p>${escapeHtml(body)}</p><div class="link-row">${primary ? textLink(prefix, primary.route, primary.label, primary.attrs ?? '') : ''}${secondary ? textLink(prefix, secondary.route, secondary.label, secondary.attrs ?? '') : ''}</div></div>
...
const renderB2B = () => {
  ...
  const body = `<main id="content">
    ${innerHero({ prefix, label: 'Интерьерным брендам', title: data.hero_title, body: data.hero_body, imagePath: 'assets/projects/dark-bathroom.webp', imageAlt: 'Продукт в интерьерной визуализации RENDART', primary: { route: '/contacts/?audience=brand&source=b2b-hero', label: 'Обсудить запуск', attrs: 'data-track="cta_click" data-block="hero"' }, secondary: { route: '/portfolio/#brands', label: 'Смотреть B2B-проекты' }, className: 'inner-hero-b2b' })}
  ...
  return pageShell({ page, route: '/b2b/', body, bodyClass: 'b2b-page', preloadImage: 'assets/projects/dark-bathroom.webp' });
};
...
const renderDesigners = () => {
  ...
  const designerCases = cases.filter((project) => project.audience === 'designer');
  const body = `<main id="content">
    ${innerHero({ prefix, label: 'Дизайнерам и архитекторам', title: data.hero_title, body: data.hero_body, imagePath: 'assets/projects/calm-bedroom.webp', imageAlt: 'Интерьерная визуализация RENDART', primary: { route: '/contacts/?audience=designer&source=designers-hero', label: 'Обсудить проект', attrs: 'data-track="cta_click" data-block="hero"' }, secondary: { route: '/portfolio/#designers', label: 'Посмотреть работы' }, className: 'inner-hero-designers' })}
    ...
    ${processList(data.process_items.map((item) => [item, '']), 'Понятный путь от брифа до сдачи')}
    ${inquiryForm(prefix, { audience: 'designer', title: data.form_title, body: data.form_body, submitLabel: 'Обсудить проект' })}
  </main>`;
  return pageShell({ page, route: '/designers/', body, bodyClass: 'designers-page', preloadImage: 'assets/projects/calm-bedroom.webp' });
};
...
const renderCase = (project, index) => {
  ...
  const body = `<main id="content" class="case-main">
    ...
    <section class="case-chapter"><div><p class="section-kicker">Проектная основа</p><h2>${escapeHtml(data.foundation)}</h2></div><figure>${image(prefix, project.images[1], `Проектная основа: ${data.task}`)}</figure></section>
    <section class="case-gallery" aria-label="Финальное решение">${project.images.map((path, imageIndex) => `<figure>${image(prefix, path, `${data.task}, материал ${imageIndex + 1}`)}</figure>`).join('')}</section>
  ...
  return pageShell({ page, route, body, bodyClass: 'case-page', prefix, ogImage: project.cover, preloadImage: project.cover, structuredData: breadcrumb });
};
...
${indexedRoutes.map((route) => `  <url><loc>${canonical(route)}</loc><lastmod>2026-08-12</lastmod></url>`).join('\n')}
</urlset>\n`);
